...
import com.wzf.tuojian.function.http.dto.request.ChangeUserInfoReqDto;
import com.wzf.tuojian.function.http.dto.request.CommitPostReqDto;
import com.wzf.tuojian.function.imageloader.ImageLoader;
import com.wzf.tuojian.function.imageloader.ImageLoaderToBitmapListener;
import com.wzf.tuojian.function.imageloader.ImagePickerImageLoader;
import com.wzf.tuojian.function.upload_qiniu.UpLoadQiNiuManager;
import com.wzf.tuojian.ui.base.BaseActivity;
import com.wzf.tuojian.utils.DebugLog;
import com.wzf.tuojian.utils.PictureUtil;
...
public class EditPostActivity extends BaseActivity {
    ...
    private void initView() {
        ...
        imRight1.setImageResource(R.drawable.selector_im_send);
...
//        imagePicker.setFocusHeight(ScreenUtils.getScreenHeight(MyApplication.getAppInstance()));
        // 图片保存的宽高800
...
//                        uploadPicture(images.get(0).path);
                        uploadPicture(PictureUtil.compressImage(images.get(0).path));
                    }
                }
                break;
        }
    ...
    private void insertPicture(final String path, final String result) {
        int targetW = (int)(ScreenUtils.getScreenWidth(MyApplication.getAppInstance()));
        Bitmap src = BitmapFactory.decodeFile(path);
        float scale = 0;
        if(src != null){
            scale = (float) src.getHeight() / (float) src.getWidth();
            src.recycle();
        }
        int height = (int)(targetW * scale);
        ImageLoader.getInstance().fileToBitmap(path,targetW, height, new ImageLoaderToBitmapListener() {
            @Override
            public void onLoadFinish(Bitmap bitmap) {
//                Bitmap scaleBitmap = PictureUtil.getBitmapByWith(bitmap, ScreenUtils.getScreenWidth(MyApplication.getAppInstance()));
